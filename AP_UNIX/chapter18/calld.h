...
#define CS_CALL "/home/philip/calld"    /* well-known name */
#define CL_CALL "call"
...
typedef struct {    /* one Client struct per connected clent */
...
typedef struct {    /* everything for one entry in Dialers file */
    char    *dialer;    /* everything for one entry in Dialers file */
    char    *sub;       /* phone number substitution string (ignored) */
    char    *expsend;   /* expect/send char */
...
void    client_del(int);
void    client_sigchild(pid_t, int);
