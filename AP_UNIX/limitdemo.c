...
{
    if (argc != 2)
        err_quit("usage: limitdemo <dirname>");
    ...
    pr_sysconf("STRAM_MAX    =", _SC_STREAM_MAX);
#endif
    ...
    pr_pathconf("_POSIX_VDISTABLE  =", "/dev/tty", _PC_VDISABLE);
    pr_pathconf("LINK MAX          =", argv[1], _PC_LINK_MAX);
